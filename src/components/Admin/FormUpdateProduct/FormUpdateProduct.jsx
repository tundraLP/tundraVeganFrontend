...
const FormUpdateProduct = ({ product, setUpdateOn, updateOn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      productToUpdate = newImageBool ? new ProductToUpdate({...input, image: newImageBool}, product) : new ProductToUpdate(input, product);
      if (productToUpdate.readyToUpdate){
        const response = await axios.put(`${uriBack}/product/updateProduct`, productToUpdate);
    ...
    <>
      <form onSubmit={handleSubmit}>
            <div className='image-input'>
                ...
                <img className='image-preview' src={product.image} alt='Profile image' id='img-preview' />
            ...
            <Input
                ...
                <div>
                    ...
                    <select onChange={handleTypeChange} className='select-product'>
                        <option key={product.Type.name} value={product.Type.name}>{product.Type.name}</option>
                        {types.map((type) => type.name != product.Type.name && <option key={type.name} value={type.name}>{type.name}</option>)}
                    </select>
            ...
            <button className='button-form' onClick={(e)=>{e.preventDefault();setUpdateOn(!updateOn)}}>Cancelar</button>
